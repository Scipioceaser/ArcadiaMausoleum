...
public class TextLerpEffects : MonoBehaviour
{
    ...
    public IEnumerator LerpText(Vector3 offset, float time, bool delayWord = false, bool delayCharacter = false, float wordDelayTime = 0.4f)
    {
        for (int i = 0; i < text.textInfo.wordCount; i++)
        {
            StartCoroutine(LerpWord(i, offset, time, delayCharacter));
    ...
    public IEnumerator LerpWord(int wordIndex, Vector3 offset, float time, bool delay = false, float characterDelaySpeed = 0.025f)
    {
        TMP_WordInfo info = text.textInfo.wordInfo[wordIndex];

        for (int i = 0; i < info.characterCount; i++)
        {
            StartCoroutine(LerpCharacter(info.firstCharacterIndex + i, offset, time));
    ...
    public IEnumerator LerpCharacter(int index, Vector3 offset, float time)
    {
        ...
        while (t < 1.0f)
        {
            ...
            newColors[vertexIndex].a = (byte)Mathf.Lerp(0, 255, Mathf.SmoothStep(0.0f, 1.0f, t));
            ...
            destinationVertices[vertexIndex] = Vector3.Lerp(destinationVertices[vertexIndex], sourceVertices[vertexIndex], Mathf.SmoothStep(0.0f, 1.0f, t));
